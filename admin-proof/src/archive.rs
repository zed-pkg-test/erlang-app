...
pub(crate) fn extract_artifact(
    ...
    if !destination.join("beam/worker.beam").is_file() {
        bail!("archive missing required beam/worker.beam");
    }
    Ok(())
}
...
pub(crate) fn validate_artifact_path(path: &Path) -> Result<()> {
    ...
    if matches!(
        path.to_str(),
        Some("manifest.json" | "admission-report.json" | "provenance.json" | "attestation.json")
    ) {
        return Ok(());
    }
