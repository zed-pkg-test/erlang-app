...
async fn main() -> io::Result<()> {
    ...
    let guest_identity = Arc::new(GuestIdentity::from_cmdline(&boot_cmdline)?);
    ...
    let supervisor_addr = Arc::new(supervisor_addr);
    let artifact_root = Arc::new(artifact_root);
    tracing::info!(
        vsock_port,
        supervisor_addr = %supervisor_addr,
        artifact_root = %artifact_root.display(),
        max_artifact_bytes,
        max_extracted_bytes,
        max_chunk_bytes,
        execution_class = %guest_identity.execution_class,
        execution_backend = %guest_identity.execution_backend,
        tenant_id = %guest_identity.tenant_id,
        runtime_epoch = guest_identity.runtime_epoch,
    ...
    loop {
        ...
        let guest_identity = guest_identity.clone();
        tokio::spawn(async move {
            if let Err(err) = proxy_once(
                stream,
                &supervisor_addr,
                &artifact_root,
                &guest_identity,
...
async fn proxy_once(
    mut vsock: tokio_vsock::VsockStream,
    supervisor_addr: &str,
    artifact_root: &Path,
    ...
    let request = read_frame(&mut vsock, max_frame_bytes).await?;
    match decode_put_request(&request) {
        Ok(Some(put)) => {
            let build_sha256 = put.build_sha256.clone();
            if let Err(message) = guest_identity.validate_fields(
                &put.execution_class,
            ...
            {
                Ok(response) => {
                    write_success_response(&mut vsock, &response, max_frame_bytes).await
                }
                Err(message) => {
                    tracing::warn!(%build_sha256, error = %message, "guest artifact transfer rejected");
                    write_error_response(&mut vsock, &build_sha256, message, max_frame_bytes).await
                }
            }
        }
        Err(message) => {
            tracing::warn!(error = %message, "invalid guest artifact transfer header");
            write_error_response(&mut vsock, "", message, max_frame_bytes).await
        }
        Ok(None) => {
            if let Err(message) = guest_identity.validate_frame(&request) {
                ...
                return write_frame(&mut vsock, &response, max_frame_bytes).await;
            }
            let mut supervisor = TcpStream::connect(supervisor_addr).await?;
            write_frame(&mut supervisor, &request, max_frame_bytes).await?;
            let response = read_frame(&mut supervisor, max_frame_bytes).await?;
            write_frame(&mut vsock, &response, max_frame_bytes).await
        }
    }
}
