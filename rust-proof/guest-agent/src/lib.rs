...
impl GuestIdentity {
    pub fn from_cmdline(cmdline: &str) -> io::Result<Self> {
        ...
        let runtime_epoch = cmdline_value(cmdline, "bmscl.runtime_epoch")?
            .parse::<u64>()
            .map_err(|_| io::Error::new(io::ErrorKind::InvalidData, "invalid bmscl.runtime_epoch"))?;
    ...
    pub fn validate_frame(&self, bytes: &[u8]) -> Result<(), String> {
        let value: Value =
            serde_json::from_slice(bytes).map_err(|err| format!("invalid guest request json: {err}"))?;
        let class = value
            .get("execution_class")
