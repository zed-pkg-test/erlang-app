...
fn validate_request(request: &PutArtifactRequest) -> Result<(), String> {
    ...
    if !matches!(
        request.archive_name.as_str(),
        "worker.zip" | "worker.tar.gz"
    ) {
        return Err("archive_name must be worker.zip or worker.tar.gz".into());
    }
...
fn validate_existing(final_dir: &Path, request: &PutArtifactRequest) -> Result<(), String> {
    ...
    for required in required_paths() {
        if !final_dir.join(required).is_file() {
            return Err(format!("existing artifact is missing required {required}"));
        }
    }
    Ok(())
}
...
fn extract_artifact(
    ...
    match archive_name {
        "worker.zip" => extract_zip(archive_path, destination, max_extracted_bytes, max_files)?,
        "worker.tar.gz" => {
            extract_tar_gz(archive_path, destination, max_extracted_bytes, max_files)?
        }
        _ => return Err("unsupported artifact archive name".into()),
    }
    for required in required_paths() {
...
fn extract_zip(
    ...
    for index in 0..archive.len() {
        ...
        write_extracted_file(destination, &path, &mut entry, size)?;
    }
    Ok(())
}
...
fn required_paths() -> [&'static str; 6] {
    [
        "manifest.json",
        "admission-report.json",
        "provenance.json",
