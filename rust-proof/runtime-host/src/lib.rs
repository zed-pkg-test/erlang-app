...
impl RuntimeHost {
    ...
    async fn boot_vm(&self, record: &ShardRecord) -> Result<RuntimeHandles, HostError> {
        ...
        let paths = RuntimePaths::new(&self.config.runtime_dir, record);
        fs::create_dir_all(&paths.dir).await?;
        remove_if_exists(&paths.api_socket).await?;
        remove_if_exists(&paths.vsock_path).await?;
        let pid = spawn_firecracker(&self.config, &paths.api_socket).await?;
        if let Err(err) = prepare_cgroup(&self.config, record, pid).await {
        ...
        if let Err(err) =
            wait_for_socket(&paths.api_socket, self.config.api_socket_timeout_ms).await
        {
            let _ = terminate_pid(pid).await;
            return Err(err);
        }
        let client = FcClient::new(paths.api_socket.clone());
        let configured = async {
            ...
            client
                .put(
                    ...
                    json!({
                        "drive_id": "rootfs",
                        "path_on_host": self.config.rootfs_image.display().to_string(),
                        "is_root_device": true,
                        "is_read_only": true
                    }),
                )
            ...
            client
                .put(
                    ...
                    json!({
                        "vsock_id": "control",
                        "guest_cid": 3,
                        "uds_path": paths.vsock_path.display().to_string()
                    }),
                )
                .await?;
            client
        ...
        Ok(RuntimeHandles {
            pid: Some(pid),
            api_socket: Some(paths.api_socket),
            vsock_path: Some(paths.vsock_path),
        })
    }
    ...
    async fn restore_vm(&self, record: &ShardRecord) -> Result<RuntimeHandles, HostError> {
        ...
        let paths = RuntimePaths::new(&self.config.runtime_dir, record);
        if !paths.snapshot_state.exists() || !paths.snapshot_mem.exists() {
        ...
        if let Err(err) = client
            .put(
                "/snapshot/load",
                json!({
                    "snapshot_path": paths.snapshot_state.display().to_string(),
                    "mem_backend": {
                        "backend_type": "File",
                        "backend_path": paths.snapshot_mem.display().to_string()
                    },
    ...
    async fn snapshot_and_stop(&self, record: &ShardRecord) -> Result<(), HostError> {
        ...
        let api = record
            .api_socket
            .clone()
            .unwrap_or_else(|| paths.api_socket.clone());
        ...
        client
            .put(
                ...
                json!({
                    "snapshot_type": "Full",
                    "snapshot_path": paths.snapshot_state.display().to_string(),
                    "mem_file_path": paths.snapshot_mem.display().to_string()
                }),
            )
            .await?;
...
async fn spawn_firecracker(config: &HostConfig, api_socket: &Path) -> Result<u32, HostError> {
    let mut child = Command::new(&config.firecracker_bin)
        ...
        .spawn()
        .map_err(|err| {
            HostError::Firecracker(format!("spawn {}: {err}", config.firecracker_bin.display()))
        })?;
    let pid = child
        .id()
        .ok_or_else(|| HostError::Firecracker("firecracker process has no pid".into()))?;
