        ...
        #{<<"op">> := <<"critical_section">>,
          ...
          when is_binary(Operation0), is_binary(TenantId), is_binary(Namespace),
            ...
            case normalize_critical_operation(Operation0) of
                ...
                Operation ->
                    LeaseMs = maps:get(<<"lease_ms">>, Envelope, undefined),
                    ...
                    case valid_critical_identity(TenantId, Namespace, ObjectKey, Holder)
                         ...
                         andalso valid_critical_arguments(Operation, LeaseMs, Token) of
                        true ->
                            {ok, #{
                                op => critical_section,
                                operation => Operation,
...
valid_critical_arguments(acquire, LeaseMs, undefined) ->
    valid_critical_lease(LeaseMs);
valid_critical_arguments(renew, LeaseMs, Token) ->
    valid_critical_lease(LeaseMs) andalso is_map(Token);
valid_critical_arguments(release, undefined, Token) ->
    is_map(Token);
valid_critical_arguments(_, _, _) ->
    false.
