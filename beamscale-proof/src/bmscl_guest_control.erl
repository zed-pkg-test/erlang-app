    ...
    case bmscl_guest_protocol:decode_request(Frame) of
        ...
        {ok, #{op := critical_section,
            ...
            critical_section_exact(
              ...
              RuntimeEpoch, Holder, LeaseMs, Token);
        {error, DecodeReason} ->
...
critical_section_exact(
  acquire, TenantId, Namespace, ObjectKey, RuntimeEpoch, Holder, LeaseMs, _Token) ->
    Result = bmscl_critical_section_registry:acquire(
               TenantId, Namespace, ObjectKey, RuntimeEpoch, Holder, LeaseMs),
    bmscl_guest_protocol:encode_critical_section_result(acquire, Result);
critical_section_exact(
  renew, TenantId, Namespace, ObjectKey, RuntimeEpoch, Holder, LeaseMs, Token) ->
...
critical_section_exact(
  release, TenantId, Namespace, ObjectKey, RuntimeEpoch, Holder, _LeaseMs, Token) ->
