...
pub struct PlacementService {
    ...
    runtime_control_secret: Option<Vec<u8>>,
}
...
impl PlacementService {
    pub fn from_env() -> Result<Self, PlacementError> {
        ...
        let runtime_control_secret = if firecracker_hosts.is_empty() {
            None
        } else {
            Some(runtime_auth::load_secret().map_err(PlacementError::Unavailable)?)
        };
        Self::new(faas_hosts, firecracker_hosts, runtime_control_secret)
    }
    ...
    async fn activate_on_host(
        ...
        let request = ActivateShardRequest {
            tenant_id: &key.tenant_id,
            shard_id: &key.shard_id,
            execution_class: key.execution_class,
            execution_backend: key.execution_class.backend(),
...
pub fn execution_class_name(class: ExecutionClass) -> &'static str {
    match class {
...
pub fn execution_backend_name(backend: ExecutionBackend) -> &'static str {
    match backend {
