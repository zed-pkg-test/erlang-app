...
struct AppState {
    ...
    runtime_control_secret: Option<Arc<Vec<u8>>>,
}
...
impl AppState {
    fn from_env() -> Result<Self, PlacementError> {
        ...
        let security = SecurityClient::from_env(runtime_client.clone())
            .map_err(|err| PlacementError::Unavailable(err.to_string()))?;
        let runtime_control_secret = runtime_auth::load_secret().ok().map(Arc::new);
        Ok(Self {
...
async fn invoke(
    ...
    let guest_response = dispatch_to_guest(
        ...
        state.runtime_control_secret.as_deref().map(Vec::as_slice),
        &runtime,
        &invocation_id,
        &deployment.worker_version_id,
        input.request,
...
async fn dispatch_to_guest(
    client: &Client,
    runtime_control_secret: Option<&[u8]>,
    ...
    let response = match runtime.execution_backend {
        ExecutionBackend::Firecracker => {
            let secret = runtime_control_secret.ok_or_else(|| {
                api_error(
